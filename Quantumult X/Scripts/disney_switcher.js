...
const $ = new Env('Disneyᐩ 策略切换')
let disneyPolicyName = $.getval('Helge_0x00.Disney_Policy') || 'Disneyᐩ'
...
;(async () => {
    ...
    if (status === STATUS_AVAILABLE) {
        ...
        $.msg($.name, `${actualNode}`, `当前节点支持 Disneyᐩ ➟ ${flag} ${regionName}`)
        return
    }
    ...
    if (cachePolicies.length <= 0) {
        throw '没有可用策略，请先运行 「Disneyᐩ 解锁检测」脚本'
    }
    ...
    $.msg($.name, `${curPolicyPath[curPolicyPath.length - 1]} ➟ ${switchPath[switchPath.length - 1]}`, `该节点支持 Disneyᐩ ➟ ${flag} ${regionName}`)
})()
    .catch(error => {
        console.log(error)
        ...
        $task.fetch(request).then(
            response => {
                ...
                if (statusCode === 302) {
                    ...
                    // 不可用
                    if (location.indexOf('Sorry, Disneyᐩ is not available in your region.') !== -1) {
                        if (debug) {
        ...
        for (let { policy, status, region, time } of results) {
            switch (status) {
                case STATUS_COMING: {
                    let flag = getCountryFlagEmoji(region) ?? ''
                    let regionName = REGIONS?.[region.toUpperCase()]?.chinese ?? ''
                    console.log(`${policy}: Disneyᐩ 即将登陆 ➟ ${flag}${regionName}`)
                    break
                }
                case STATUS_AVAILABLE: {
                    let flag = getCountryFlagEmoji(region) ?? ''
                    let regionName = REGIONS?.[region.toUpperCase()]?.chinese ?? ''
                    console.log(`${policy}: 支持 Disneyᐩ ➟ ${flag}${regionName}`)
                    availablePolicies.push({ policy, region, time })
                    break
                }
                case STATUS_NOT_AVAILABLE:
                    console.log(`${policy}: 不支持 Disneyᐩ`)
                    break
                case STATUS_TIMEOUT:
...
function lookupTargetPolicy(policies = {}) {
    ...
    if (policyNames.length === 1) {
        ...
        throw '没有找到 Disneyᐩ 策略组，请在 BoxJS 中填写正确的策略组名称'
    } else {
        throw `找到多个 Disneyᐩ 策略组，请在 BoxJS 中填写正确的策略组名称`
    }
}
